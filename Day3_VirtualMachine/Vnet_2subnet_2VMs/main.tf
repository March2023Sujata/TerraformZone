...
resource "azurerm_public_ip" "public_ip" {
  count               = length(var.vnet_info.subnets_names)
  name                = "VM_public_ip-${var.vnet_info.subnets_names[count.index]}"
  resource_group_name = azurerm_resource_group.ntierRG.name
  location            = azurerm_resource_group.ntierRG.location
...
resource "azurerm_network_interface" "ntireNIC" {
  count               = length(var.vnet_info.subnets_names)
  name                = "ntireNic-${var.vnet_info.subnets_names[count.index]}"
  location            = azurerm_resource_group.ntierRG.location
  resource_group_name = azurerm_resource_group.ntierRG.name
...
resource "azurerm_linux_virtual_machine" "main" {
  count                           = length(var.vnet_info.subnets_names)
  name                            = "ntireVM-${var.vnet_info.subnets_names[count.index]}"
  location                        = azurerm_resource_group.ntierRG.location
  resource_group_name             = azurerm_resource_group.ntierRG.name
  ...
  os_disk {
    name                 = "myOSdisk-${var.vnet_info.subnets_names[count.index]}"
